...
public static class ErpClientRegistration
{
    ...
    public static IServiceCollection AddErpApi(this IServiceCollection services)
    {
        ...
        ordinary.AddTypedClient(static (httpClient, sp) =>
        {
            ...
            return (IErpClient)new ErpClient(httpClient, agent, null, longPoll.SendAsync);
        });
    ...
    /// <summary>Registers the standard (non-long-poll) ERP pipeline only.</summary>
    public static IHttpClientBuilder AddErpClient(this IServiceCollection services)
    {
        var builder = services.AddHttpClient<IErpClient, ErpClient>(ErpHttpClientName, (sp, client) => ConfigureErpClient(client, sp.GetRequiredService<IOptions<ErpOptions>>().Value));
