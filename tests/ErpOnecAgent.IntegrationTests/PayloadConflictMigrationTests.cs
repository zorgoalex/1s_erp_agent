...
public sealed class PayloadConflictMigrationTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Populated_v3_upgrade_preserves_unfinished_work_and_is_idempotent()
    {
        ...
        var oldLedgerBefore = await SnapshotRowsAsync("SELECT version,name,checksum,applied_at_utc FROM schema_migrations WHERE version<=3 ORDER BY version;");
        ...
        Assert.Equal(oldLedgerBefore, await SnapshotRowsAsync("SELECT version,name,checksum,applied_at_utc FROM schema_migrations WHERE version<=3 ORDER BY version;"));
        ...
        Assert.Equal(1, await CountAsync("SELECT COUNT(*) FROM results_outbox WHERE status='pending'"));
        Assert.Equal(7, await CountAsync("SELECT COUNT(*) FROM commands_inbox WHERE status IN ('queued','retry_waiting','unknown_result','executing','result_pending')"));
        Assert.Equal(0, await CountAsync("SELECT COUNT(*) FROM command_payload_conflicts"));
        Assert.Equal(0, await CountAsync("SELECT COUNT(*) FROM pragma_foreign_key_list('command_payload_conflicts')"));
        ...
        var eventsAfterFirstRun = await SnapshotRowsAsync("SELECT * FROM command_payload_conflicts ORDER BY event_id;");
        ...
        Assert.Equal(eventsAfterFirstRun, await SnapshotRowsAsync("SELECT * FROM command_payload_conflicts ORDER BY event_id;"));
        Assert.Equal(commandsBefore, await SnapshotRowsAsync("SELECT * FROM commands_inbox ORDER BY command_id;"));
        Assert.Equal(attemptsBefore, await SnapshotRowsAsync("SELECT * FROM command_attempts ORDER BY attempt_id;"));
        Assert.Equal(outboxBefore, await SnapshotRowsAsync("SELECT * FROM results_outbox ORDER BY result_id;"));
        Assert.Equal(12, await CountAsync("SELECT COUNT(*) FROM schema_migrations"));
