...
public sealed class EtlSendAttemptsMigrationTests : IAsyncLifetime
{
    ...
    public async Task Populated_v7_database_upgrades_additively_preserving_every_row_and_evidence()
    {
        ...
        var runsBefore = await SnapshotRowsAsync($"SELECT {RunColumnsV7} FROM etl_runs ORDER BY run_id;");
        ...
        Assert.Equal(runsBefore, await SnapshotRowsAsync($"SELECT {RunColumnsV7} FROM etl_runs ORDER BY run_id;"));
